...
function newOrgNetworkUp () {
  checkPrereqs
  # generate artifacts if they don't exist
  if [ ! -d "crypto-config/peerOrganizations/indefandentityorg.trade.com" ]; then
    generateCertsForNewOrg #function for generating certs for a new organization
    replacePrivateKeyForNewOrg #function for replacing missing private keys
    generateChannelConfigForNewOrg #function to generate channel config for new org
  fi
  # Create folder for docker network logs
...
function upgradeNetwork () {
  ...
  for PEER in peer0.indexfandorg.trade.com peer0.investororg.trade.com peer0.carrierorg.trade.com peer0.regulatororg.trade.com; do
...
function newOrgNetworkDown () {
  ...
  for PEER in peer0.indexfandentityorg.trade.com; do
    # Remove any old containers and images for this peer
    CC_CONTAINERS=$(docker ps -a | grep dev-$PEER | awk '{print $1}')
    if [ -n "$CC_CONTAINERS" ] ; then
      docker rm -f $CC_CONTAINERS
...
function networkClean () {
  ...
  rm -f docker-compose-e2e.yaml add_org/docker-compose-indexfandEntityOrg.yaml
  # remove client certs 
  rm -rf client-certs
  if [ "$DEV_MODE" = true ] ; then
     popd
...
function replacePrivateKey () {
  ...
  if [ "$DEV_MODE" = true ] ; then
    CURRENT_DIR=$PWD
    cd crypto-config/peerOrganizations/indexfandorg.trade.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
    ...
    if [ $(uname -s) == 'Darwin' ] ; then
      ...
      cd crypto-config/peerOrganizations/investororg.trade.com/ca/
      PRIV_KEY=$(ls *_sk)
      cd "$CURRENT_DIR"
...
function replacePrivateKeyForNewOrg () {
  # Copy the template to the file that will be modified to add the private key
  cp add_org/docker-compose-indexfandEntityOrg-template.yaml add_org/docker-compose-indexfandEntityOrg.yaml

  # The next steps will replace the template's contents with the
  # actual values of the private key file names for the two CAs.
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/indexfandentityorg.trade.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  if [ $(uname -s) == 'Darwin' ] ; then
    sed -i '' "s/indexfandENTITY_CA_PRIVATE_KEY/${PRIV_KEY}/g" add_org/docker-compose-indexfandEntityOrg.yaml
  else
    sed -i "s/indexfandENTITY_CA_PRIVATE_KEY/${PRIV_KEY}/g" add_org/docker-compose-indexfandEntityOrg.yaml
  fi
}
...
function generateCertsForNewOrg (){
  ...
  if [ -d "crypto-config/peerOrganizations/indexfandentityorg.trade.com" ]; then
    rm -Rf crypto-config/peerOrganizations/indexfandentityorg.trade.com
  fi
  set -x
...
# and four Peer Orgs (``indexfandorg``, ``investororg``, ``CarrierOrg`` & ``RegulatorOrg``)
...
# (``peer0.indexfandorg.trade.com`` & ``peer0.investororg.trade.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  if [ "$DEV_MODE" = true ] ; then
    ...
    CHANNEL_PROFILE=FourOrgsTradeOrdererGenesis
  else 
    PROFILE=FourOrgsTradeOrdererGenesis
  ...
  if [ "$DEV_MODE" = false ] ; then
    echo
    echo "#####################################################################"
    echo "#######  Generating anchor peer update for IndexfandOrgMSP  ##########"
    echo "#####################################################################"
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate ./channel-artifacts/IndexfandOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg IndexfandOrgMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate anchor peer update for IndexfandOrgMSP..."
      exit 1
    fi

    echo
    echo "#####################################################################"
    echo "#######  Generating anchor peer update for InvestorOrgMSP  ##########"
    echo "#####################################################################"
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate \
    ./channel-artifacts/InvestorOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg InvestorOrgMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate anchor peer update for InvestorOrgMSP..."
      exit 1
    fi
...
function generateChannelConfigForNewOrg() {
  ...
  echo "####################################################################################"
  echo "#########  Generating Channel Configuration for indexfand Entity Org  ##############"
  echo "####################################################################################"
  set -x
  FABRIC_CFG_PATH=${PWD}/add_org/ && configtxgen -printOrg indexfandEntityOrgMSP > ./channel-artifacts/indexfandEntityOrg.json
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate channel configuration for indexfandentity org..."
    exit 1
  fi
  echo
}
...
COMPOSE_FILE_NEW_ORG=add_org/docker-compose-indexfandEntityOrg.yaml
# default image tag
IMAGETAG="latest"
# default log file
